...
import { computePlacementMetrics } from '/home/andyblocker/scpper-cn/user-backend/src/routes/gacha/runtime.ts';
...
function instanceHeuristic(inst) {
  const styles = inst.styles;
  return (BASE_BY_RARITY[inst.card.rarity] ?? 0) * 10 + styleHeuristic(styles) + (styles.includes('NONE') ? -0.6 : 0);
}
...
function evaluate(slotInsts, addonInst, stateObj) {
  const metrics = computePlacementMetrics(userId, stateObj, buildSlots(slotInsts), buildAddons(addonInst));
  return metrics;
}

function keyOfState(slotInsts, addonInst) {
  const slotIds = slotInsts.map((s) => s.id).sort().join(',');
  const addonId = addonInst ? addonInst.id : 'NONE';
  return `${slotIds}|${addonId}`;
}
...
function localOptimize(slotInstsInit, addonInit, slotPool, addonPool, stateObj, maxPasses = 20) {
  let slotInsts = [...slotInstsInit];
  let addonInst = addonInit;

  let { addon: bestAddon, metrics: bestMetrics } = pickBestAddon(slotInsts, addonPool, stateObj, addonInst);
  addonInst = bestAddon;
  ...
  for (let pass = 0; pass < maxPasses; pass += 1) {
    ...
    for (let idx = 0; idx < slotInsts.length; idx += 1) {
      ...
      if (bestCand.id !== current.id) {
        ...
        improved = true;
        const addonRes = pickBestAddon(slotInsts, addonPool, stateObj, addonInst);
        addonInst = addonRes.addon;
        bestMetrics = addonRes.metrics;
      }
    }

    const addonRes = pickBestAddon(slotInsts, addonPool, stateObj, addonInst);
    if ((addonInst?.id ?? null) !== (addonRes.addon?.id ?? null)
      || addonRes.metrics.estimatedYieldPerHour > bestMetrics.estimatedYieldPerHour + 1e-9) {
      addonInst = addonRes.addon;
      bestMetrics = addonRes.metrics;
      improved = true;
    }
...
(async () => {
  ...
  if (!placementState) {
    throw new Error('No placement state for user');
  }

  const currentPlacementRows = await prisma.gachaPlacementSlot.findMany({
    where: { userId },
  ...
  if (goldPool.length < 10) {
    throw new Error(`Not enough GOLD instances: ${goldPool.length}`);
  }
  if (addonPool.length < 1) {
    throw new Error('No COLORLESS addon candidates');
  }
  ...
  const currentSlots = currentPlacementRows
    ...
    .filter(Boolean);
  const currentAddon = currentPlacementRows
    .find((r) => r.slotIndex === 0 && r.instanceId);
  const currentAddonInst = currentAddon ? allInstances.find((x) => x.id === currentAddon.instanceId) : null;

  const seeds = [];
  if (currentSlots.length === 10) {
    seeds.push({ slotInsts: currentSlots, addonInst: currentAddonInst ?? sortedAddon[0] });
  }

  seeds.push({ slotInsts: sortedGold.slice(0, 10), addonInst: sortedAddon[0] });
  seeds.push({ slotInsts: sortedGold.slice(0, 9).concat(sortedGold.slice(20, 21)), addonInst: sortedAddon[0] });

  for (let i = 0; i < 60; i += 1) {
    const slotInsts = sampleUnique(goldPool, 10);
    const used = new Set(slotInsts.map((s) => s.id));
    const addonCand = sortedAddon.find((a) => !used.has(a.id)) ?? sortedAddon[0];
    seeds.push({ slotInsts, addonInst: addonCand });
  }
  ...
  for (const seed of seeds) {
    const key = keyOfState(seed.slotInsts, seed.addonInst);
    if (seen.has(key)) continue;
    seen.add(key);

    const optimized = localOptimize(seed.slotInsts, seed.addonInst, goldPool, addonPool, placementState, 18);
  ...
  if (!best) {
    throw new Error('Optimizer failed to produce result');
  }

  // Single-swap global check against full inventory (allows non-GOLD upgrades if any)
  let improved = true;
  let swapPasses = 0;
  while (improved && swapPasses < 3) {
    improved = false;
    swapPasses += 1;

    // Re-opt addon first
    const addonRes = pickBestAddon(best.slotInsts, addonPool, placementState, best.addonInst);
    best.addonInst = addonRes.addon;
    best.metrics = addonRes.metrics;
    ...
    for (let idx = 0; idx < best.slotInsts.length; idx += 1) {
      ...
      if (localBestInst.id !== current.id) {
  ...
  // Sort slots for easier human execution: by rarity desc, title, signature
  const finalSlots = [...best.slotInsts].sort((a, b) => {
    const rar = ['WHITE', 'GREEN', 'BLUE', 'PURPLE', 'GOLD'];
    const ra = rar.indexOf(a.card.rarity);
    const rb = rar.indexOf(b.card.rarity);
