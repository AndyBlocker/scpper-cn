    ...
    >
      <div class="flex items-center gap-3 min-w-0 relative w-full" :class="variant === 'lg' ? 'pr-14' : ''" v-if="variant !== 'sm'">
        ...
        <div :class="rankPositionClass">
          <div v-if="rank != null" class="text-base font-semibold text-[var(--g-accent)] dark:text-[var(--g-accent)] tabular-nums leading-none">#{{ rank }}</div>
      ...
      <template v-if="variant === 'lg'">
        ...
        <div v-if="categoryRanksNorm.length" class="mt-3 grid grid-cols-2 gap-2 w-full">
          <div v-for="r in categoryRanksNorm" :key="r.name" class="border border-neutral-200 dark:border-neutral-800 rounded px-2 py-1 text-xs flex items-center justify-between">
            ...
            <span class="font-semibold text-[var(--g-accent)] dark:text-[var(--g-accent)] tabular-nums">#{{ r.rank }}</span>
          </div>
        </div>
      </template>
...
import { navigateTo } from 'nuxt/app'
  ...
  const rootClass = computed(() => {
    ...
    if (variant.value === 'lg') return [baseClass, to.value ? 'hover:shadow-md cursor-pointer focus:outline-none focus:ring-2 ring-[var(--g-accent)]' : '', 'p-5 md:p-6 flex flex-col relative'].join(' ')
    if (variant.value === 'md') return [baseClass, to.value ? 'hover:shadow-md cursor-pointer focus:outline-none focus:ring-2 ring-[var(--g-accent)]' : '', 'p-3 flex items-center justify-between gap-2'].join(' ')
    return ['relative self-start justify-self-start inline-flex items-center gap-1.5 rounded-full border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900', to.value ? 'hover:shadow-sm cursor-pointer focus:outline-none focus:ring-2 ring-[var(--g-accent)]' : '', 'px-2 py-1 min-h-[26px] w-auto', props.smBgClass || ''].join(' ')
  })
  ...
  const nameClass = computed(() => {
    if (variant.value === 'lg') return 'text-base font-medium text-[var(--g-accent)]'
    if (variant.value === 'md') return 'text-[13px] font-medium text-[var(--g-accent)]'
    return 'text-[12px] font-medium text-[var(--g-accent)]'
  })
  
  const nameSmClass = computed(() => (props.smTextClass && props.smTextClass.trim()) ? props.smTextClass : 'text-[12px] leading-none font-medium text-[var(--g-accent)] truncate max-w-[140px]')
