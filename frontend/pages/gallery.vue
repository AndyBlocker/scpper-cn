...
const galleryImages = computed(() => {
  return galleryImagesRaw.value
    ...
    const orientation = ratio == null ? 'unknown' : ratio >= 2.2 ? 'panorama' : ratio <= 0.45 ? 'tall' : ratio >= 1.4 ? 'landscape' : ratio <= 0.75 ? 'portrait' : 'normal';
...
definePageMeta({ key: 'page-image-gallery' });
</script>
